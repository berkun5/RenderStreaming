  ...
  async setupConnection(useWebSocket) {
    ...
    if (useWebSocket) {
      if (ipPort == -1) {
        ...
        this.signaling = new WebSocketSignaling(ipPort);
      }
    } else {
    ...
    // Create data channel with proxy server and set up handlers
    this.multiplayChannel.onopen = function () {
      Logger.log('Datachannel connected.');
    };
    this.multiplayChannel.onerror = function (e) {
      Logger.log("The error " + e.error.message + " occurred\n while handling data with proxy server.");
    };
    this.multiplayChannel.onclose = function () {
      Logger.log('Datachannel disconnected.');
    };
    this.multiplayChannel.onmessage = async (msg) => {
      console.log('RECEIVED A MESSAGE1.');
